    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-transparent backdrop-blur-2xl  md:text-sm ${isMenuOpen ? "shadow-lg rounded-xl border mt-2 md:shadow-none md:border-none md:mx-2 md:mt-0" : ""}`}>
      <div className="gap-x-14 items-center w-full mx-0 px-4 md:flex md:px-8">
        <div className="flex items-center justify-between py-5 md:block">
          <a href="/">
            <img className="invert scale-125" src={Logo} width={120} height={50} alt="Resourcify logo" />
          </a>
          <div className="md:hidden">
            <button className="menu-btn text-gray-500 hover:text-gray-800" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {isMenuOpen ? (
        ...
        <div className={`flex-1 items-center mt-8 md:mt-0 md:flex ${isMenuOpen ? "block" : "hidden"} menu-content`}>
